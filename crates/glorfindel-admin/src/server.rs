...
pub async fn run(addr: &str, data_dir: PathBuf) -> anyhow::Result<()> {
    ...
    *state.definitions.write().await = persist::load_definitions(&data_dir).await;
    ...
    let api = Router::new()
        ...
        .route("/graph/nodes/:id", axum::routing::patch(graph::patch_node).delete(graph::delete_node))
        .route("/graph/edges", get(graph::list_edges).post(graph::create_edge))
        .route("/graph/edges/:id", axum::routing::patch(graph::patch_edge).delete(graph::delete_edge))
